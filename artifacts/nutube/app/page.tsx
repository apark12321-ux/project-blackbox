...
// 정적 가이드 38편 (sitemap.ts와 동기화)
const STATIC_GUIDE_COUNT = 38;
...
export default function HomePage() {
  ...
  const [totalCount, setTotalCount] = useState(STATIC_GUIDE_COUNT);
  ...
  const [popularPosts, setPopularPosts] = useState(POPULAR);

  // Upstash에서 전체 가이드 개수 가져오기
  // 38편은 이미 마이그레이션 완료 → API total = 전체 개수
  useEffect(() => {
    fetch('/api/posts?limit=200')
      ...
      .then(json => {
        if (json.success && json.data) {
          // 전체 개수
          setTotalCount(json.data.total || json.data.posts?.length || STATIC_GUIDE_COUNT);
          
          // 카테고리별 개수 (API 응답에서 직접 카운트)
          if (Array.isArray(json.data.posts)) {
            const newCounts: Record<string, number> = {
              algorithm: 0, senior: 0, aitools: 0, monetization: 0
            };
            json.data.posts.forEach((p: any) => {
              if (p.category && newCounts[p.category] !== undefined) {
                newCounts[p.category]++;
              }
            });
            ...
            // LATEST: 최신 발행일 기준 상위 3개
            const sorted = [...json.data.posts].sort((a: any, b: any) => 
              (b.publishedAt || '').localeCompare(a.publishedAt || '')
            );
            const latest = sorted.slice(0, 3).map((p: any) => ({
              slug: p.slug,
              cat: catLabelMap[p.category] || p.categoryLabel || '',
              catKey: p.category,
              title: p.title,
            ...
            // POPULAR: 알고리즘 카테고리 글 상위 3개 (또는 임의 고정)
            const algoPosts = sorted.filter((p: any) => p.category === 'algorithm').slice(0, 3);
            if (algoPosts.length >= 3) {
              const popular = algoPosts.map((p: any) => ({
                slug: p.slug,
                cat: catLabelMap[p.category] || p.categoryLabel || '',
                catKey: p.category,
                title: p.title,
          ...
          <div className="nh-hero-inner">
            ...
            <div className="nh-hero-card">
              ...
              <div className="nh-hero-floating">
                ...
                <div>
                  <p className="nh-hero-floating-text">실전 가이드 {totalCount}편</p>
          ...
          <div className="nh-cat-grid">
            {CATEGORIES.map((c) => (
              <Link key={c.id} href={`/blog?cat=${c.id}`} className="nh-cat-card">
                ...
                <div className="nh-cat-count">가이드 {catCounts[c.id] ?? c.count}편 →</div>
              </Link>
            ))}
          </div>
        </section>
