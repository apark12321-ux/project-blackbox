...
const DOMAIN_PROFILES: Record<DomainType, DomainProfile> = {
  ...
  language: {
    type: 'language',
    primaryTriggers: ['경험담', '시간효율', '거울', '강의수익화'],
    numbers: {
      primary: ['토익 850점', '토익 900점', '말이 트인', '외국인과 30분 대화', '6개월 만에 회화', '강의로 월 5천만원', '수강생 35,000명'],
    ...
    mirror: [
      ...
      '학원 등록부터 하시는 분들 많은데요',
      '회화는 막상 입이 안 떨어지죠',
      '영어 잘해도 월급 250만원에서 끝나시잖아요',
      '내 영어 능력으로 돈 벌 생각 안 하시죠',
    ],
    secrets: [
      '비결은 단어암기가 아니라 패턴이에요',
      '진짜 중요한 건 문법이 아니라 발화량이에요',
      '핵심은 학원이 아니라 매일 5분의 반복이에요',
      '진짜 답은 영어 잘하기가 아니라 영어로 강의 만들기예요',
      '영어 능력 + 채널 = 월 수억 원도 가능해요',
    ],
    audience: ['30대 직장인', '40대 중년 학습자', '대학생', '취준생', '영어 강사 지망생', '한국어 강사 (외국인 대상)'],
  ...
  selfdev: {
    type: 'selfdev',
    primaryTriggers: ['시간효율', '평범인변신', '비밀', '지식수익화'],
    numbers: {
      primary: ['하루 30분 투자', '6개월 변화', '독서 100권', '2배 효율', '강의로 월 1천만원', '전자책 1억 매출'],
    ...
    secrets: [
      ...
      '진짜 답은 능력이 아니라 채널 + 외부 수익 연결이에요',
      '같은 능력이라도 누구는 월 250만원, 누구는 수십억원입니다',
      '비결은 본업이 아니라 지식 콘텐츠로 전환하는 거예요',
    ],
    audience: ['30대 직장인', '40대 자기계발러', '대학생', '직장인 부모', '부업 시작자', '지식 창업가'],
    emphasis: ['직접 해보니', '1년 동안', '실제로', '솔직히'],
  },

  aitech: {
    type: 'aitech',
    primaryTriggers: ['무료', '무제한', '비싼것대체', '외부수익', '팬덤활용'],
    numbers: {
      primary: ['빵원', '0원', '무제한 사용', '10분 작업', '23분 만에', '구독자 4천명으로 2,500만원', '6개월 만에 1억'],
    ...
    mirror: [
      ...
      'ChatGPT만 쓰시잖아요',
      '유료 도구 결제부터 고민하시죠',
      '조회수 수익만 바라보고 시작하시잖아요',
      '구독자 1만 명까지 키워야 한다고 생각하시죠',
    ],
    secrets: [
      '비결은 도구가 아니라 프롬프트예요',
      '진짜 중요한 건 비싼 툴이 아니라 사용법이에요',
      '핵심은 AI가 아니라 워크플로우예요',
      '진짜 답은 조회수 수익이 아니라 외부 수익이에요',
      '구독자 4천명만으로도 충분합니다, 진짜 찐팬이라면',
      '비결은 채널 키우기가 아니라 시청자를 고객으로 바꾸는 거예요',
    ],
    audience: ['크리에이터 지망생', '자영업자', '디지털 입문자', '40대 IT 활용', '부업 시작자', '본업 부업자'],
...
function buildHook(
  ...
  const hookByDomain: Record<DomainType, string[]> = {
    ...
    aitech: [
      ...
      // 외부 수익화 패턴
      `구독자 4천명 채널이 250만 채널보다 더 법니다. ${keyword} 활용한 외부 수익으로 6개월에 2,500만원. 조회수 수익에 매달리지 마세요. 진짜 답은 외부 수익에 있어요.`,
      `${keyword}, 조회수 수익만 보면 망합니다. 100만 조회수 = 20만 원이지만, 외부 수익 연결하면 = 3억 원. ${detail.audience}${jong(detail.audience, "이라면", "라면")} 꼭 알아야 할 차이예요.`,
      `${keyword}, 구독자 0명부터 바로 수익화 됩니다. 수익 창출 조건 달성 안 됐어도 OK. 비결은 외부 수익 연결이에요. 오늘 그 5가지 경로 정리해드립니다.`,
      // AI 워크플로우
      `${keyword}, AI 4단계 워크플로우면 1시간에 영상 1개 가능합니다. 트렌드 분석 → 대본 생성 → 이미지 → 영상화. ${detail.audience}${jong(detail.audience, "도", "도")} 따라할 수 있어요.`,
      `${keyword}, AI 영상 만들 때 캐릭터가 자꾸 바뀌시죠? 비결은 한 장이 아니라 3장을 학습시키는 거예요. 일관성이 영상의 운명을 결정합니다.`,
      // 팬덤 요소
      `${keyword} 영상이 떡상하는 진짜 이유, 팬덤 요소예요. 일반 캐릭터 < 팬덤 캐릭터. 시청자 참여율이 100배 차이 납니다. ${detail.emphasis} 알게 된 핵심 정리해드릴게요.`,
    ],
    senior: [
